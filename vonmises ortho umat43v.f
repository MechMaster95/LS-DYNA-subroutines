...
c******************************************************************
     ...
     1 y35(nlq),y46(nlq),z17(nlq),z28(nlq),z35(nlq),z46(nlq)
	  ...
	  INTEGER :: ifail
	  REAL*8 :: smat,tw	  
      LOGICAL failels(*),reject,ierr,tfail,res1,res2,res3,res4,res5
      CHARACTER*5 etype
	  REAL*8 ::
...
c	  common/bk20loc/idummy(10),ndum
c
...
c  S1-S6 will be local stresses in Ea,Eb,Ec orientations
        ...
        S5 = 2.*C55*G5
        S6 = 2.*C66*G6
c First 32 slots for history variables(hsv)are unused
...
c  Calculating the local stresses 
   ...
   7  continue
      ...
      call tsaiwu(sigloc,smat,tfail,tw)
        if(tfail .eqv. .true.) then		
        failels(i) = .true.
c******************************************************************
c		print*, 'failel parameter', failels(i)
c		print*, 'deleting parameter', ifail(i)
c		nelm = nhxbwp(nlq*(ndum-1)+i)
c		print*, 'the user element is :',nelm
c		print*, 'The internal element id is',i
c		print*, 'The ndum value is', ndum
c       write(* ,1990) mxe, (fmode(kk),kk=1,9)
c       write(86,1990) mxe, (fmode(kk),kk=1,9)		
c		write(6,*) 'Element no', i ,'Failed at time =  ', tt
c		print*, 'failel parameter', failels(i)
c		print*, 'deleting parameter', ifail(i)
c******************************************************************
		end if
...
c
      subroutine tsaiwu(sig,smat,tfail,tw)   
c  subroutine to calculate Tsai-Wu failure criteria
      REAL*8 :: sig,smat
      DIMENSION :: sig(6)
c      DIMENSION ::smat(12)
c      REAL*8 :: F1,F2,F3,F11,F22,F33,F12,F23,F13,F44,F55,F66
      REAL*8 :: N1,N2,N3,N4,N5,N6,tw
      LOGICAL :: tfail
c Tsai wu failure criteria theory
c******************************************************************
c      F1 = (1.0/smat(1))-(1.0/smat(4))
c      F2 = (1.0/smat(2))-(1.0/smat(5))
c      F3 = (1.0/smat(3))-(1.0/smat(6))
c      F11 = 1.0/(smat(1)*smat(4))
c      F22 = 1.0/(smat(2)*smat(5))
c      F33 = 1.0/(smat(3)*smat(6))
c      F44 = 1.0/smat(7)**2
c      F55 = 1.0/smat(8)**2
c      F66 = 1.0/smat(9)**2
c      F12 = (2.0/smat(10)**2)-((F1+F2)/smat(10))-0.5*(F11+F22+F44)
c      F23 = (2.0/smat(11)**2)-((F3+F2)/smat(11))-0.5*(F33+F22+F55)
c      F13 = (2.0/smat(12)**2)-((F1+F3)/smat(12))-0.5*(F11+F33+F66)

c      N1 = F1*sig(1)+F2*sig(2)+F3*sig(3)-1.0
c      N2 = F11*sig(1)**2+F22*sig(2)**2+F33*sig(3)**2
c      N3 = 2.0*F12*sig(1)*sig(2)
c      N4 = 2.0*F23*sig(2)*sig(3)
c      N5 = 2.0*F13*sig(1)*sig(3)
c      N6 = F44*sig(4)**2+F55*sig(5)**2+F66*sig(6)**2
c      tw = N1+N2+N3+N4+N5+N6
c******************************************************************
c Von mises failure criteria
	   ...
	   N5 = sig(5)**2
	   N6 = sig(6)**2
	   tw = (smat**2)-0.5*(N1+N2+N3+6*(N4+N5+N6))
c******************************************************************
      if(tw .le. 0.0 ) then
      tfail = .true.
      else
