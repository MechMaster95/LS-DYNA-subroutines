...
c  subroutine to calculate Tsai-Wu failure criteria
      ...
      DIMENSION :: sig(6)
      DIMENSION ::smat(12)
      REAL*8 :: F1,F2,F3,F11,F22,F33,F12,F23,F13,F44,F55,F66
      REAL*8 :: N1,N2,N3,N4,N5,N6,tw
      LOGICAL :: tfail
c Tsai wu failure criteria theory
c******************************************************************
       F1 = (1.0/smat(1))-(1.0/smat(4))
       F2 = (1.0/smat(2))-(1.0/smat(5))
       F3 = (1.0/smat(3))-(1.0/smat(6))
       F11 = 1.0/(smat(1)*smat(4))
       F22 = 1.0/(smat(2)*smat(5))
       F33 = 1.0/(smat(3)*smat(6))
       F44 = 1.0/smat(7)**2
       F55 = 1.0/smat(8)**2
       F66 = 1.0/smat(9)**2
       F12 = (2.0/smat(10)**2)-((F1+F2)/smat(10))-0.5*(F11+F22+F44)
       F23 = (2.0/smat(11)**2)-((F3+F2)/smat(11))-0.5*(F33+F22+F55)
       F13 = (2.0/smat(12)**2)-((F1+F3)/smat(12))-0.5*(F11+F33+F66)
...
c	    N5 = sig(5)**2
c	    N6 = sig(6)**2
c	    tw = (smat**2)-0.5*(N1+N2+N3+6*(N4+N5+N6))
c******************************************************************
      if(tw .gt. 0.0 ) then
      tfail = .true.
      else
